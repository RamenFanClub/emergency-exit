...
| F78 | Add JWT token expiry | Must | backlog | **OWASP A02 — Cryptographic Failures.** `create_token()` sets no `exp` claim — tokens never expire. A stolen token grants permanent access. Add `"exp": datetime.utcnow() + timedelta(hours=24)` and `"iat"` to payload. PyJWT auto-rejects expired tokens. ~10 min. **Tier 1.** |
...
| F88 | Lock down CORS to allowed origins only | Must | backlog | **OWASP A05 — Security Misconfiguration.** `allow_origins=["*"]` with `allow_credentials=True` lets any website make authenticated API calls as a logged-in user. Change to `["https://kinlight.app", "https://ramenfanclub.github.io"]`. ~5 min. **Tier 1.** |
