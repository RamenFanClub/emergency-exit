...
- **Test suite:** `identity-service/test_main.py` — 164 pytest tests covering all backend features (will be 175 once F04 lands)
...
| F91 | Rate limiting on all endpoints | Should | backlog | **OWASP A07 — Auth Failures.** No rate limiting anywhere. Enables brute-force login, nomination email spam, Resend quota exhaustion. **Confirmed June 2026 review:** `POST /auth/request-reset` sends a real email on every call for any existing username with no cooldown — highest-priority instance of this gap; can spam a tester's inbox or exhaust the Resend 100/day free tier with a simple loop. Add `slowapi`: login 5/min per IP, reset 3/min, nominate 10/min per user, admin 5/min. ~1 hr. **Tier 2.** |
...
| F99 | Plaintext vault cached indefinitely in browser localStorage | Should | backlog | **Found in June 2026 end-to-end review.** `index.html` caches the full decrypted vault blob (assets, will, letters, contacts) under `localStorage` key `ee_v3` as an offline fallback when the server fetch fails. Unlike `sessionStorage` (used correctly for the JWT token), `localStorage` persists indefinitely — surviving browser restarts — and isn't cleared on logout. On a shared or stolen device, this is a real plaintext exposure path. Note this is independent of F04: encrypting vault content in MongoDB doesn't and shouldn't touch this, since the browser legitimately needs plaintext to render the UI — the gap is what happens to that plaintext *after* it's rendered. Options to evaluate: drop the offline-fallback feature entirely, clear `ee_v3` on logout, add a TTL, or encrypt the local cache with a device-derived key (adds real complexity). Needs a product decision on how much offline support matters before implementing. |
